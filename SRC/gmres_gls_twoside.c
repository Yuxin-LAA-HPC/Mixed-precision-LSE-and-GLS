...
{
    // As and Bs store the GQR factorization result of Ao and Bo.
    ...
    *num_iter = 0;
    ...
    // Require double space 9*n + n*m + 6*m + m*m
    m = restrt;
    rtmp = work;
    r = rtmp + n;
    V = r + n;
    ...
    funcAxres_gls(n, mo, no, po, Ao, lda, Bo, ldb, alpha, x, r);//rtmp-A*x
    over = tic();
    time[1] = over-start+time[1];
    start = tic();
    funcpretwoside_left_gls_p(mo, no, po, As, ldas, Bs, ldbs, T2s, alpha,
        workssub, r, worksub);
    over = tic();
    ...
    for (int iter = 0; iter < maxiter; iter++)
    {
        ...
        for (i = 0; i < m; i++)
        {
            ...
            *num_iter = *num_iter + 1;
            ...
            funcpretwoside_right_gls_p(mo, no, po, As, ldas, Bs, ldbs, T2s,
            alpha, workssub, vcur, worksub);
            funcAx_gls(n, mo, no, po, Ao, lda, Bo, ldb, alpha, vcur, w);//A*x
            over = tic();
            time[1] = over-start+time[1];
            start = tic();
            funcpretwoside_left_gls_p(mo, no, po, As, ldas, Bs, ldbs, T2s,
            ...
            if (error <= tol)
            {
                ...
                dgemv_("N", &n, &i, &one, V, &n, y, &incx, &zero, vec_tmp,
                        &incx, 1);
                funcpretwoside_right_gls_p(mo, no, po, As, ldas, Bs, ldbs,
                        T2s, alpha, workssub, vec_tmp, worksub);
        ...
        dgemv_("N", &n, &m, &one, V, &n, y, &incx, &zero, vec_tmp,
                &incx, 1);
        funcpretwoside_right_gls_p(mo, no, po, As, ldas, Bs, ldbs, T2s, alpha,
        workssub, vec_tmp, worksub);
        daxpy_(&n, &one, vec_tmp, &incx, x, &incx);
    ...
    double *work, int lwork)
{
    ...
    *num_iter = 0;
    ...
    time[1] = over-start+time[1];
    start = tic();
    funcpretwoside_left_gls_n(mo, no, po, As, ldas, Bs, ldbs, T2s, alpha,
        workssub, r, worksub);
    over = tic();
    time[2] = over-start+time[2];
    ...
    for (int iter = 0; iter < maxiter; iter++)
    {
        ...
        for (i = 0; i < m; i++)
        {
            ...
            *num_iter = *num_iter + 1;
            dcopy_(&n, &V[i*n], &incx, vcur, &incx);
            funcpretwoside_right_gls_n(mo, no, po, As, ldas, Bs, ldbs, T2s,
            alpha, workssub, vcur, worksub);
            funcAx_gls(n, mo, no, po, Ao, lda, Bo, ldb, alpha, vcur, w);//A*x
            over = tic();
            time[1] = over-start+time[1];
            start = tic();
            funcpretwoside_left_gls_n(mo, no, po, As, ldas, Bs, ldbs, T2s,
                    alpha, workssub, w, worksub);
            over = tic();
            time[2] = over-start+time[2];
            start = tic();
            ...
            if (error <= tol)
            {
                ...
                dgemv_("N", &n, &i, &one, V, &n, y, &incx, &zero, vec_tmp,
                        &incx, 1);
                funcpretwoside_right_gls_n(mo, no, po, As, ldas, Bs, ldbs,
                        T2s, alpha, workssub, vec_tmp, worksub);
                daxpy_(&n, &one, vec_tmp, &incx, x, &incx);
                break;
            }
        ...
        dgemv_("N", &n, &m, &one, V, &n, y, &incx, &zero, vec_tmp,
                &incx, 1);
        funcpretwoside_right_gls_n(mo, no, po, As, ldas, Bs, ldbs, T2s, alpha,
        workssub, vec_tmp, worksub);
        daxpy_(&n, &one, vec_tmp, &incx, x, &incx);
