...
int mplse_gmres_twoside_scal(int m, int n, int p, double *A, int lda,
    double *B, int ldb, double *c, double *d, double *xout,
    double *work, float *works, int lwork)
{
#ifdef MEASURETIME
    printf("%% Measure time\n");
    double start, over;
    ...
    double norma, normb, tol = 3.0*1e-16;
    ...
    for (int i = 0; i < n; i++)
        b_aug[m+p+i] = 0.0;
    gmres_lse_twoside_scal(length, m, n, p, A, lda, B, ldb, As_d, m, Bs_d, p,
