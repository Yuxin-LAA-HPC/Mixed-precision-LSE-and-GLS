...
*>
*  =====================================================================
      SUBROUTINE SGELSY_QZ( M, N, NRHS, A, LDA, B, LDB, JPVT, RCOND,
     $                   RANK, Q1s, Zts,
     $                   WORK, LWORK, INFO )
*
...
*     .. Array Arguments ..
      INTEGER            JPVT( * )
      REAL               A( LDA, * ), B( LDB, * ), WORK( * )
      REAL               Q1s(M, N), Zts(M, N), Bc( M, NRHS )
*      DOUBLE PRECISION   Q1(M, N), Zt(N, N), WORKD( LWORK ), AD( M, N )
*     ..
*
*  =====================================================================
*
...
*
*      CALL SLAG2D( M, N, A, LDA, Q1, M, INFO )
*      CALL SLAG2D( MN, 1, WORK( 1 ), MN, WORKD( 1 ), MN, INFO )
*     MY: AD = Q1 := Q * I(1:M,1:RANK)---Q1:M*RANK
*      CALL DORGQR( M, N, MN, Q1, M, WORKD( 1 ), WORKD( 2*MN+1 ),
*     $             LWORK-2*MN, INFO )
      CALL SLACPY( 'A', M, N, A, LDA, Q1s, M )
      CALL SORGQR( M, N, MN, Q1s, M, WORK( 1 ), WORK( 2*MN+1 ),
     $             LWORK-2*MN, INFO )
*      CALL SLACPY( 'A', M, NRHS, B, LDB, WORK( 2*MN+1 ), M )
      CALL SGEMM( 'T', 'N', RANK, NRHS, M, ONE, Q1s, M,
     $             B, LDB, ZERO, Bc, M )
...
*
      IF( RANK.LT.N ) THEN
*         CALL SLAG2D( M, N, A, LDA, AD, M, INFO )
*         CALL SLAG2D( MN, 1, WORK( MN+1 ), MN, WORKD( 1 ), MN, INFO )
*         CALL DLASET( 'A', N, RANK, 0.0D+0, 1.0D+0, Zt, N )
*         CALL DORMRZ( 'Left', 'Transpose', N, RANK, RANK, N-RANK, AD, M,
*     $                WORKD( 1 ), Zt, N, WORKD( 2*MN+1 ), LWORK-2*MN,
*     $                INFO )
         CALL SLASET( 'A', N, RANK, ZERO, ONE, Zts, N )
         CALL SORMRZ( 'Left', 'Transpose', N, RANK, RANK, N-RANK, A,
     $                LDA, WORK( MN+1 ), Zts, N, WORK( 2*MN+1 ),
     $                LWORK-2*MN, INFO )
         CALL SGEMM( 'N', 'N', N, NRHS, RANK, ONE, Zts, N, Bc, M,
     $                ZERO, B, LDB)
